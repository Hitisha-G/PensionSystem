...
import { log_in } from "../redux/action/userAction";
...
const Login = (props) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if(formData.type === 'Organization'){
        const register =    await pension.methods.LoginO(email,password).send({from: formData.EAddress});
        console.log({register})
        await log_in(formData);
  ...
  <div className="login_head">Loginn As {formData.type}</div>
    ...
    {formData.type !== 'Organization' &&  <div onClick={()=> setformData({...formData, type:"Organization"})  } className="registerLisk" >Login As a Organization </div> }
    {formData.type !== 'User' &&  <div onClick={()=> setformData({...formData, type:"User"})  } className="registerLisk" >Login As a User </div> }
    {formData.type !== 'Validatir' &&  <div onClick={()=> setformData({...formData, type:"Validatir"})  } className="registerLisk" >Login As a Validatir </div> }
